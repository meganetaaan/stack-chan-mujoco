...
p=argparse.ArgumentParser(description=__doc__);p.add_argument('--mesh-mm',choices=['1','0.7','0.5'],default='0.7');p.add_argument('--penalty',type=float,default=1e5);p.add_argument('--out',type=Path,required=True);a=p.parse_args()
root=Path(__file__).resolve().parents[3]
if not np.isfinite(a.penalty) or a.penalty<=0:p.error('positive finite penalty required')
if a.out.exists():p.error('new output required')
a.out.mkdir(parents=True)
source=root/f'validation/boot_nut_patch_development_v1/seat_{a.mesh_mm}.msh';m=meshio.read(source);xyz=m.points;tet=m.cells_dict['tetra'];tri=m.cells_dict['triangle'];tags=m.cell_data_dict['gmsh:physical']['triangle']
...
lines+=['*SURFACE,NAME=MASTER,TYPE=ELEMENT','SUPPORT,S2','*SURFACE INTERACTION,NAME=NORMAL','*SURFACE BEHAVIOR,PRESSURE-OVERCLOSURE=LINEAR',str(a.penalty),'*CONTACT PAIR,INTERACTION=NORMAL,TYPE=SURFACE TO SURFACE','SLAVE,MASTER','*NSET,NSET=ANCHORS',f'{na},{nb}','*BOUNDARY','SUPPORT,1,3,0',f'{na},1,2,0',f'{nb},2,2,0','*STEP,NLGEOM,INC=100','*STATIC','.1,1,1e-6,.1','*CLOAD']+[f'{n},3,{f:.15g}' for n,f in loads.items()]
