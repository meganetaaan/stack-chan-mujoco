...
Development by default. Reserved acceptance seeds require a complete immutable
manifest; the same servo physics and qualification rules apply in both modes.
"""
import argparse
import gzip
import json
...
def main():
    ...
    p.add_argument('--acceptance-manifest',type=Path,help='frozen admission for reserved full-schedule acceptance trials')
    ...
    p.add_argument('--record-contact-trace',action='store_true',help='record loads and landing tracker values every physics step')
    p.add_argument('--physics-dt-s',type=float,choices=[.001,.0005,.00025],default=.001,help='integration convergence diagnostic; control period remains 20 ms')
    a=p.parse_args()
    ...
    acceptance=None
    if a.acceptance_manifest:
        from yaw_acceptance import validate_trial
        acceptance=validate_trial(a)
    elif 217000<=a.seed<=217019 or 218000<=a.seed<=218019:p.error('reserved acceptance seed')
    ...
    states=[state()];torques=[];targets=[];observations=[];actions=[];imu_observations=[];contact_trace=[]
    ...
    while not failure and d.time<a.duration-1e-9:
        ...
        for _ in range(substeps):
            ...
            if a.record_contact_trace:
                contact_trace.append([float(d.time),*plant.loads,*heights,*tracker.airtime,*tracker.ground_time,*tracker.peak,*tracker.counts])
    ...
    if a.record_contact_trace:
        np.savez_compressed(a.out/'contact_trace.npz',values=np.array(contact_trace).reshape(-1,13),
                            columns=np.array(['time_s','left_load_N','right_load_N','left_height_m','right_height_m',
                                              'left_airtime_s','right_airtime_s','left_ground_time_s','right_ground_time_s',
                                              'left_peak_m','right_peak_m','left_landings','right_landings']))
    if protocol:
    ...
    report={'scope':__doc__,'design':str(a.design),'reference':str(a.reference),'requested_duration_s':a.duration,
            'formal_acceptance':acceptance is not None,'acceptance':acceptance,
            'physics_dt_s':float(m.opt.timestep),'effective_motor_delay_s':bank.delay_ticks*m.opt.timestep,
    ...
    if a.record_contact_trace:report['contact_trace_sha256']=sha(a.out/'contact_trace.npz')
    report['acceptance_pass']=bool(acceptance is not None and report.get('development_trial_pass',False))
    if acceptance is not None and validate_trial(a)!=acceptance:raise ValueError('acceptance snapshot changed during trial')
