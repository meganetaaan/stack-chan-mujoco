...
def full_body(sign=1,drive_model="current",model_path=None,turn_inset_mm=25.5,**parameters):
    if not np.isfinite(turn_inset_mm) or not 20 <= turn_inset_mm < 26:
        raise ValueError("turn inset must be in [20,26) mm")
    class Reference(FastTurnReference):
        initial_inset_mm=turn_inset_mm
        heading_goal_rad=sign*np.deg2rad(91.5)
        def __init__(self,*args):
            args=list(args);args[5:11]=[.45,turn_inset_mm,.4,.4,'quartic',None]
            super().__init__(*args)
    ...
    model=Path(model_path) if model_path is not None else SIM/'assets/r9_fast_turn_v1'
