"""Cut boot openings for the ankle bridge and rear lower corner; attachment strength remains unverified."""
...
plan={'opening_roll_frame_mm':{'x':[-31.5,3],'y':[-32,32],'z':[15,45]},'rear_inner_relief_mm':{'x':[-31,-29],'y':[-18,18],'z':[-14,0]},'criteria':{'valid_single_solid':True,'unchanged_below_z_mm':-15},'source_sha256':{str(f.relative_to(root)):hashlib.sha256(f.read_bytes()).hexdigest() for f in paths},'limitations':['Candidate opening only; edge radii, boot stiffness and whole assembly clearance remain unverified.']}
(a.out/'plan.json').write_text(json.dumps(plan,indent=2)+'\n');rows=[]
cut=cq.Workplane('XY').box(34.5,64,30).val().translate((-14.25,0,30))
cut=cut.fuse(cq.Workplane('XY').box(2,36,14).val().translate((-30,0,-7)))
protected=cq.Workplane('XY').box(200,200,200).val().translate((0,0,-115))
