...
p=argparse.ArgumentParser(description=__doc__);p.add_argument('--gimbal-dir',type=Path);p.add_argument('--out',type=Path,required=True);a=p.parse_args()
...
gimbal_dir=a.gimbal_dir.resolve() if a.gimbal_dir else root/'validation/ankle_gimbal_relief_development_v1/v4/cad'
...
for side in ('left','right'):
 ...
 yoke=cq.importers.importStep(str(yoke_dir/f'{side}_foot_yoke.step')).val()
 cross_pairs.append(dict(side=side,distance_mm=gimbal.distance(yoke),overlap_mm3=gimbal.intersect(yoke).Volume()))
 for i,part in enumerate(parts):
...
report=dict(yoke_gimbal_pairs=cross_pairs,gimbal_rows=gimbal_rows,gimbal_overlap_findings=[r for r in gimbal_rows if not r['overlap_pass']],rows=rows,overlap_findings=[r for r in rows if not r['overlap_pass']],manufacturing_release=False)
