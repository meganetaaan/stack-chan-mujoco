"""Nominal M3 fastening and tool envelopes for the reinforced yaw support.
...
p.add_argument('--stage',choices=['complete','before_yaw_and_power'],default='complete')
a=p.parse_args()
...
updated=a.support_dir.resolve() if a.support_dir else ROOT/'validation/yaw_back_reinforcement_development_v1/yaw_back_reinforced_v2'
parts={}
...
parts['rear_cover']=cq.importers.importStep(str(base/'rear_cover.step')).val()
pattern=[[0,dy,dz] for dz in [-12,12] for dy in [-a.pattern_half_width_mm,a.pattern_half_width_mm]]
for sign in [1,-1]:
 for _,dy,dz in pattern:
  parts['rear_cover']=parts['rear_cover'].cut(cq.Solid.makeCylinder(1.7,5,cq.Vector(-65,sign*26+dy,70+dz),cq.Vector(1,0,0)))
cq.exporters.export(parts['rear_cover'],str(a.out/'rear_cover_drilled.step'))
deferred=[n for n in parts if n.endswith(('yaw_coupler','yaw_motor_case')) or n in ('dedicated_5V_converter','TTL_interface')] if a.stage=='before_yaw_and_power' else []
for name in deferred:del parts[name]
plan=dict(scope=__doc__,stage=a.stage,deferred_parts=deferred,pattern_relative_mm=pattern,
          source_sha256={str(f.relative_to(ROOT)):hashlib.sha256(f.read_bytes()).hexdigest() for f in [*base.glob('*.step'),*updated.glob('*.step')]},
          bolt_length_mm=a.bolt_length_mm,intersection_limit_mm3=.01,tool_residual_clearance_mm=.5,
...
for side in ['left','right']:
 for k,(_,dy,dz) in enumerate(pattern):
  ...
  for kind,shape in envelopes.items():
   ...
   intersections=[];near=[];closest={'part':None,'distance_mm':float('inf')}
   for part,fixed in parts.items():
    distance=float(shape.distance(fixed))
    if distance<closest['distance_mm']:closest={'part':part,'distance_mm':distance}
    if distance<1e-6:
   ...
   checks.append({'envelope':name,'intersections':intersections,'tool_clearance_findings':near,'closest_part':closest})
...
report=dict(scope=__doc__,stage=a.stage,deferred_parts=deferred,rows=rows,checks=checks,
