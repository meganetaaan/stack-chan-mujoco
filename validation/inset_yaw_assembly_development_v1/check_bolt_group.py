...
def main():
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--out',type=Path,required=True);p.add_argument('--config',type=Path)
    a=p.parse_args();a.out.mkdir(parents=True,exist_ok=False)
    config_path=a.config or ROOT/'board/mechanical/engineering/yaw_fasteners.json'
