"""Bound a moving ankle part/gimbal clearance over a continuous ankle-roll interval."""
import argparse
import hashlib
import json
import math
...
p.add_argument('--moving-dir',type=Path)
p.add_argument('--moving-part',choices=['foot_yoke','boot_shell'],default='foot_yoke')
...
paths={s:{'gimbal':a.gimbal_dir/f'{s}_ankle_gimbal.step',
          'yoke':(a.moving_dir or root/'validation/native_horn_yoke_development_v1/v4')/f'{s}_{a.moving_part}.step'} for s in ('left','right')}
plan={'moving_part':a.moving_part,'range_rad':[-.34,.34],'required_nominal_clearance_mm':1.1,
      ...
      'limitations':['Selected rigid CAD pair only; other parts, hardware, other leg, floor and harness excluded.',
