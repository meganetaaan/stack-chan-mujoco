...
def analyze_regions_many(mesh, fixed_selector, regions, load_cases, young=1400., poisson=.35, support_mode="clamped"):
    ...
    if support_mode == 'clamped':
        constrained = basis.get_dofs(facets=fixed).all()
    elif support_mode == 'normal_z':
        support = basis.get_dofs(facets=fixed)
        zdofs = support.all(['u^3'])
        if not len(zdofs):
            raise ValueError('No normal support dofs')
        if np.ptp(basis.doflocs[2, zdofs]) > 1e-6:
            raise ValueError('normal_z requires a horizontal support plane')
        # Minimal in-plane rigid-body anchors: ux,uy at A and uy at B.
        xcoords = basis.doflocs[0, zdofs]
        za, zb = zdofs[np.argmin(xcoords)], zdofs[np.argmax(xcoords)]
        if abs(basis.doflocs[0, zb]-basis.doflocs[0, za]) < 1e-6:
            raise ValueError('In-plane anchor baseline is degenerate')
        constrained = np.unique(np.r_[zdofs, za-2, za-1, zb-1])
    else:
    ...
    for wrenches in load_cases:
        ...
        report = {'support_mode':support_mode,'dofs':int(basis.N),'elements':int(mesh.nelements),'fixed_facets':len(fixed),'loaded_facets':sum(r[3] for r in region_data),'load_regions':region_reports,
...
def analyze_many(mesh, fixed_selector, loaded_selector, origin, wrenches, young=1400., poisson=.35, support_mode="clamped"):
    ...
    yield from analyze_regions_many(mesh, fixed_selector, [(loaded_selector, origin)],
                                    (np.asarray(w)[None, :] for w in wrenches), young, poisson, support_mode)


def analyze(mesh, fixed_selector, loaded_selector, origin, wrench, young=1400., poisson=.35, support_mode="clamped"):
    """Single-load wrapper retaining the original public interface."""
    return next(analyze_many(mesh, fixed_selector, loaded_selector, origin, [wrench], young, poisson, support_mode))
