...
**接続候補を追加。製作HOLD。** STM32G030F6P6と周辺6部品を追加し、電池保護側82部品、
全体205部品とした。旧SYS側シーケンサー候補とは別の役割・別の電源・別の端子割当である。
...
| 6 | NRST | 10kΩプルアップ・100nF、外部監視回路は未実装 |
| 7 | PA0 | BQ_ALLOW_BMINUS、既存1kΩ直列・10kΩBOTHOFFプルダウンへ |
| 8 | PA1 | BQ_AUX_START_REQUEST、10kΩプルダウン。境界ドライバー未実装 |
| 9 | PA2 | BQ_ALERT_N |
| 17 | PA12 | 独立監視回路用のハートビート予約、10kΩプルダウン |
...
- 外部電源監視リセット。POR/PDRだけでBQとの通信可能電圧を保証しない。
- CPU停止・出力High固着を上書きできる独立停止／監視回路。MCUのGPIO直結だけでは満たさない。
