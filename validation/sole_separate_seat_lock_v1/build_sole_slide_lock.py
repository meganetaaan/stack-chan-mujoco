...
p=argparse.ArgumentParser(description=__doc__);p.add_argument('--out',type=Path,required=True);p.add_argument('--rigid-seat',action='store_true');p.add_argument('--separate-seat',action='store_true');a=p.parse_args();root=Path(__file__).resolve().parents[3];a.out.mkdir(parents=True,exist_ok=False)
...
for side,cy in [('left',6),('right',-6)]:
 ...
 sole=parts['sole'].cut(cylinder(2.2 if a.rigid_seat else 1.15,-22.01,3.02,x,y)).cut(cylinder(2.4,-22.01,seat+22.01,x,y)).clean()
 ...
 if a.rigid_seat and not a.separate_seat:yoke=yoke.fuse(cylinder(2,seat,-19-seat+.01,x,y)).cut(cylinder(1.15,seat-.01,-11.59-seat+.01,x,y)).clean()
 ...
 if a.separate_seat:
  assert a.rigid_seat
  objects['spacer']=cylinder(2,seat,-19-seat,x,y).cut(cylinder(1.15,seat-.01,-19-seat+.02,x,y))
