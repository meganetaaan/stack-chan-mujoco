...
def main():
    ...
    a=p.parse_args()
    if a.randomize and (a.policy or a.attitude_gain or a.attitude_rate_gain):p.error('randomized observation feedback is not implemented in this development runner')
    ...
    states=[state()];torques=[];targets=[];observations=[];actions=[]
    ...
    if a.policy or a.attitude_gain or a.attitude_rate_gain:
    ...
    while not failure and d.time<a.duration-1e-9:
        ...
        if a.attitude_gain or a.attitude_rate_gain:
            ...
            correction=a.attitude_gain*angles+a.attitude_rate_gain*obs[33:35]*10
    ...
    np.savez_compressed(a.out/'states.npz',state=np.array(states),target=np.array(targets),last_substep_torque=np.array(torques),
                        legacy_observation=np.array(observations),legacy_action=np.array(actions),motion_xy_heading=np.array(motion),state_spec=int(STATE_SPEC))
    ...
    report={'scope':__doc__,'design':str(a.design),'reference':str(a.reference),'requested_duration_s':a.duration,
            ...
            'attitude_feedback':{'angle_gain':a.attitude_gain,'rate_gain_s':a.attitude_rate_gain,'input':'noisy gravity and gyro from legacy observation view'},
