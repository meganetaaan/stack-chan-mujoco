...
def main():
    ...
    p.add_argument('--gate-clamp',action='store_true',help='Add a piecewise-linear zener sensitivity scenario')
    ...
    if a.gate_clamp:
        c.update(zener_knee_V=7.2+.0045*125,zener_slope_ohm=80.,zener_capacity_F=200e-12)
        gates.update(gate_abs_max_V=10.,zener_peak_power_max_W=.1,pullup_peak_power_max_W=.25)
    plan={'gate_clamp':a.gate_clamp,'scope':__doc__,'case':a.case,'corner':a.corner,'parameters':c,'criteria':gates,
          'source_path':str(source.relative_to(ROOT)),'source_sha256':hashlib.sha256(source.read_bytes()).hexdigest(),
          'sources':{'gate_zener':'https://assets.nexperia.com/documents/data-sheet/BZT52H_SER.pdf',
                     'comparator':'https://www.ti.com/lit/ds/symlink/lm393.pdf',
                     'reference':'https://www.ti.com/lit/ds/symlink/tl431.pdf'},
          'limitations':['zener slope extrapolation and temperature shift are sensitivity assumptions, not guaranteed I-V bounds',
                         'zener model omits leakage below knee and negative gate transients',
                         'gate RC and comparator lag are engineering scenarios, not guaranteed delays',
...
'''
    if a.gate_clamp:
        brake+=f"Bzener gate 0 I=max(0,(V(gate)-{c['zener_knee_V']})/{c['zener_slope_ohm']})\nCzener gate 0 {c['zener_capacity_F']}\n"
    ...
    zener_i=np.maximum(0,(gate-c['zener_knee_V'])/c['zener_slope_ohm']) if a.gate_clamp else np.zeros_like(gate)
    if a.gate_clamp:
        checks.update(gate_voltage=bool(np.abs(gate).max()<=gates['gate_abs_max_V']),
                      zener_power=bool((gate*zener_i).max()<=gates['zener_peak_power_max_W']),
                      pullup_power=bool(((bus-gate)**2/2200).max()<=gates['pullup_peak_power_max_W']))
    if not regen:checks['no_normal_braking']=energy_brake<=gates['normal_brake_energy_max_J']
    report={'gate_clamp':a.gate_clamp,'maximum_gate_V':float(gate.max()),
            'zener_peak_power_W':float((gate*zener_i).max()),
            'pullup_peak_power_W':float(((bus-gate)**2/2200).max()),'scope':__doc__,'case':a.case,'corner':a.corner,'gates':checks,'passed_screen':all(checks.values()),
