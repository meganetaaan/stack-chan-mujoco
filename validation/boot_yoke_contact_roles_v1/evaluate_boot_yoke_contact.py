...
import numpy as np
p=argparse.ArgumentParser(description=__doc__);p.add_argument('--source',type=Path,required=True);a=p.parse_args();text=(a.source/'contact.dat').read_text();plan=json.loads((a.source/'plan.json').read_text());run=json.loads((a.source/'run.json').read_text())
