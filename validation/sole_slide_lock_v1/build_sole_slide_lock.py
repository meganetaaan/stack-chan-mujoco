...
plan={'scope':__doc__,'screw_envelope':{'diameter_mm':2,'length_mm':8,'head_diameter_mm':3.8,'head_height_mm':1.3,'seat_z_mm':-20.5},'nut_envelope_mm':[4,4,1.2],'nut_bottom_z_mm':-14.2,'criteria':{'single_valid_parts':True,'overlap_max_mm3':.01,'head_ground_recess_mm':.2},'limitations':['Nominal envelopes only; M2x8 low-head screw not selected or tolerance-qualified.','TPU head bearing and PETG boss strength, preload, creep and nut print insertion remain unverified.','Sliding stops only after installation; friction not credited.']};(a.out/'plan.json').write_text(json.dumps(plan,indent=2)+'\n');rows=[]
...
for side,cy in [('left',6),('right',-6)]:
 ...
 sole=parts['sole'].cut(cylinder(1.15,-22.01,3.02,x,y)).cut(cylinder(2.4,-22.01,1.51,x,y)).clean()
 ...
 screw=cylinder(1,-20.5,8,x,y).fuse(cylinder(1.9,-21.8,1.3,x,y)).clean();nut=cq.Solid.makeBox(4,4,1.2,cq.Vector(x-2,y-2,-14.2)).cut(cylinder(1,-14.21,1.22,x,y));tool=cylinder(1.5,-35,13.19,x,y)
