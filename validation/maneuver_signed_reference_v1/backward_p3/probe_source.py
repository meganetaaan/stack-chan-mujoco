...
def main():
    ...
    period=.27;dt=.02
    PARAMS['gait'].update(step_length_m=a.speed*period,step_height_m=.004,initial_stand_s=1.,
                         shift_s=.35*period,swing_s=.55*period,settle_s=.1*period,com_inset_mm=a.com_inset_mm)
    planner=MovingCOMReference(Planner(steps=a.steps),pose,smooth,SIDES,a.com_forward_offset_mm/1000)
