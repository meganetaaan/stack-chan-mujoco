...
p.add_argument('--primary-open-regen',action='store_true')
p.add_argument('--ldo-model',choices=['legacy','continuous'],default='legacy')
p.add_argument('--ldo-transition-v',type=float,default=.1)
a=p.parse_args();assert np.isfinite(a.ldo_transition_v) and a.ldo_transition_v>0;a.out.mkdir(parents=True,exist_ok=False)
...
plan={'scope':__doc__,'case':a.case,'ldo_model':a.ldo_model,'ldo_transition_V':a.ldo_transition_v,
 'continuous_ldo':{'target_lower_V':3.3*.99-.01-.05,'current_limit_A':.32,'reference_tau_s':.0002,'feedback_resistance_ohm':.1,'dropout_assumed_V':.65},'primary_open_regen':a.primary_open_regen,
 ...
 'Continuous LDO UVLO ramp width is a sensitivity parameter, not a specified TPS709 UVLO characteristic',
 'Continuous target combines DC accuracy, line and load limits conservatively; 200 us RC approximates 600 us to 95 percent but does not certify transient behavior',
...
if a.ldo_model=='continuous':
 net=re.sub(r'^Bldo.*$',f'''Bldotarget ldotarget 0 V=min(3.207,max(V(vin)-.65,0))*min(max((V(vin)-2.7)/{a.ldo_transition_v},0),1)
Rldolag ldotarget ldofilter 1000
Cldolag ldofilter 0 200n
Bldo vin aux I=min(.32,max(0,(V(ldofilter)-V(aux))/.1))''',net,flags=re.M)
