...
plan = {'scope': __doc__, 'parts': ['body', 'rear_plate'], 'mesh_mm': a.mesh_mm,
        ...
        'limitations': ['mesh preparation only; no contact or preload solve', 'linear tetrahedral geometry',
                        'hardware and thread connector not included'],
        'audit_sha256': hashlib.sha256((audit/'report.json').read_bytes()).hexdigest(),
        'assets': config['assets']}
(a.out/'plan.json').write_text(json.dumps(plan, indent=2)+'\n')
rows = []
for part in plan['parts']:
    source = ROOT/config['assets'][part]['path']
    assert hashlib.sha256(source.read_bytes()).hexdigest() == config['assets'][part]['sha256']
