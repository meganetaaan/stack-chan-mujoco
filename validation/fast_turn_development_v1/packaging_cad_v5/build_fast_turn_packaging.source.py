...
def main():
    ...
    # Real underside relief for the widened thigh sweep, mirrored both sides.
    for sign in [-1, 1]:
        shapes['body_shroud'] = shapes['body_shroud'].cut(box((17,8,4),(14.5,sign*49,.9)))
        shapes['body_shroud'] = shapes['body_shroud'].cut(box((6,6,4),(23,sign*8,.9)))
        # Cut mating slots instead of double-counting overlapping printed stock.
        side='left' if sign==1 else 'right'
        shapes['body_shroud'] = shapes['body_shroud'].cut(shapes[side+'_yaw_fixed_support'])
        shapes['body_shroud'] = shapes['body_shroud'].cut(cyl(1.1,6,(29,sign*59,63),(0,sign,0)))
    shapes['body_shroud'] = shapes['body_shroud'].clean()
