...
def screen(scene, spacing_mm, angle_deg, samples, axis_x_mm=-25, foot_length_mm=94, inner_trim_mm=0, battery_dx_mm=0, side_wall_tray=False):
            ...
            for angle in args.angles_deg:
                for length in args.foot_length_mm:
                    for inner_trim in args.inner_trim_mm:
                        row = screen(scene, spacing, angle, 11, axis, length, inner_trim, args.battery_dx_mm, args.side_wall_tray)
                        rows.append(row)
