...
import numpy as np
p=argparse.ArgumentParser(description=__doc__);p.add_argument('--out',type=Path,required=True);a=p.parse_args();root=Path(__file__).resolve().parents[3];a.out.mkdir(parents=True,exist_ok=False);src=root/'validation/foot_dynamics_capture_v1/foot_loads.jsonl.gz'
sys.path.insert(0,str(root/'software/sim/actuator'));from fixtures import full_body
sim,_=full_body();model=sim.m;hierarchy=[]
...
plan={'source_sha256':hashlib.sha256(src.read_bytes()).hexdigest(),'criteria':{'force_residual_N':1e-8,'moment_residual_Nmm':1e-5},'hierarchy':hierarchy,'axes':'ankle_roll body local axes; positions mm, forces N, moments N mm','limitations':['Rigid frozen foot; finite contact areas and TPU deformation are not inferred.','Wrench at yoke plane preserves resultant only, not traction distribution.','Entire foot inertia cannot be applied solely to yoke material.']};(a.out/'plan.json').write_text(json.dumps(plan,indent=2)+'\n');times=[];loads=[];errors=[];selected=None
