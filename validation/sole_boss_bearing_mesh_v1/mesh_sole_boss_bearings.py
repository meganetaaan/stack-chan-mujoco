...
p=argparse.ArgumentParser(description=__doc__);p.add_argument('--out',type=Path,required=True);a=p.parse_args();root=Path(__file__).resolve().parents[3];a.out.mkdir(parents=True,exist_ok=False);src=root/'validation/sole_lock_boss_screen_v1/boss.step';expected={'nut_bearing':16-math.pi*1.15**2,'spacer_bearing':math.pi*(4-1.15**2)}
plan={'scope':__doc__,'source_sha256':hashlib.sha256(src.read_bytes()).hexdigest(),'mesh_mm':[1,.7,.5],'expected_area_mm2':expected,'criteria':{'volume_error_mm3':1e-6,'cad_area_error_mm2':1e-6,'mesh_area_relative_error':.01},'limitations':['Nominal footprints only; edge chamfers, contact opening and friction not modeled.']};(a.out/'plan.json').write_text(json.dumps(plan,indent=2)+'\n');rows=[]
for h in plan['mesh_mm']:
 gmsh.initialize()
 try:
  gmsh.option.setNumber('General.Terminal',0);vs=[v for v in gmsh.model.occ.importShapes(str(src)) if v[0]==3];before=sum(gmsh.model.occ.getMass(*v) for v in vs);nut=gmsh.model.occ.addRectangle(33,4,-14.2,4,4);seat=gmsh.model.occ.addDisk(35,6,-19,2,2);_,mapping=gmsh.model.occ.fragment(vs,[(2,nut),(2,seat)]);gmsh.model.occ.synchronize();vols=gmsh.model.getEntities(3);boundary={t for d,t in gmsh.model.getBoundary(vols,oriented=False) if d==2};after=sum(gmsh.model.occ.getMass(*v) for v in vols);assert len(vols)==1 and abs(after-before)<1e-6
  ...
  gmsh.model.mesh.generate(3);path=a.out/f'boss_{h}.msh';gmsh.write(str(path))
