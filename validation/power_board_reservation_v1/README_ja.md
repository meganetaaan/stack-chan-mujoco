...
横置き60×70 mm案は左右ヨー支持部・取付板等と干渉し不採用。

寸法・位置はplan.json。基板領域と部品領域を既存8群の個別ソリッドと検査。基準は体積干渉なし、最小距離0.5 mm以上の公称比較。既存STEPのハッシュを照合してから実行した。

```sh
LD_LIBRARY_PATH="$PWD/.tools/root/usr/lib/x86_64-linux-gnu" .venv-engineering/bin/python software/sim/structural/check_power_board_reservation.py --layout horizontal --out /tmp/board-reservation-v1
```
