# 足部の過去比較候補 revA（現行ABS/PETG工程には未適合）

過去のTPU案の左右組立STEPと部品出典をrevA/に保存。現行工程向け再設計は[MATERIAL_CHANGE_ja.md](MATERIAL_CHANGE_ja.md)を参照。各側はブーツ、外付けナット用ヨーク、TPU足裏、スペーサー、10 mmねじ、外付けナットの6部品。サーボ/ホーン締結品/ハーネスは未収録。全身完成CADではない。
