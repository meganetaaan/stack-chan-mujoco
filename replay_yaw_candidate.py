...
def main():
    ...
    if 'physics_dt_s' in report:model.opt.timestep=report['physics_dt_s']
    data=mujoco.MjData(model)
    if 'parameters' in report:
        from stackchan_rl.candidate_variation import apply_model_parameters
        if sha('stackchan_rl/candidate_variation.py')!=report['variation_source_sha256']:
            raise ValueError('plant variation source mismatch')
        aids=np.array([model.actuator(name+'_motor').id for name in report['joint_order']])
        apply_model_parameters(model,data,aids,model.body('base').id,report['parameters'])
    ...
    metadata={'source':'recorded MuJoCo integration states','synthesized_robot_motion':False,
              ...
              'randomized':report.get('randomized',False),'parameters':report.get('parameters'),
