...
def compare(old: dict, new: dict) -> dict:
    ...
    if not old.get('config') or not new.get('config'):
        raise ValueError('Re-evaluate both checkpoints with v3 --config configs/walk_refine.json before comparison')
    ...
    for cmd in oa['commands']:
        ...
        for f in ('forward_m','pitch_rate_rms_rad_s','action_delta_rms','body_pitch_peak_to_peak_deg','peak_sole_load_bw'):
