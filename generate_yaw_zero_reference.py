...
def configure_candidate_mass(legacy,inertials,robot):
    for name in legacy.INERTIALS:
        if name!='base':legacy.INERTIALS[name]=inertials[name]
    merged=[]
...
def main():
    ...
    if not np.isfinite(a.com_inset_mm) or not 0<=a.com_inset_mm<26:p.error('COM inset must stay within the 26 mm half sole width')
    ...
    PARAMS['gait'].update(step_length_m=a.speed*a.period,step_height_m=a.step_height_mm/1000,initial_stand_s=1.,
                         shift_s=.35*a.period,swing_s=.55*a.period,settle_s=.1*a.period,com_inset_mm=a.com_inset_mm)
