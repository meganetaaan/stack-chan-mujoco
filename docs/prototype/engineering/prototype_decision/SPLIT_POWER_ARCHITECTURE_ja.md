...
```mermaid
flowchart TD
    B[電池/BMS 未選定] --> P[入力ヒューズ・逆接保護 未確定]
    P --> L[左5V変換器 候補]
    P --> R[右5V変換器 候補]
    L --> CL[左遮断・逆流保護 未完成]
    R --> CR[右遮断・逆流保護 未完成]
...
| 低残量・充電中 | 部品許容条件に従う | 電池/BMS・充電器未確定、動作許可しない |

## 候補と予算

D42V55F5×2とTPSM63610RDFR×2を比較中。後者の静的精度改善は、
2S低残量・熱・過渡の適合を意味しない。最終購入BOMではない。
