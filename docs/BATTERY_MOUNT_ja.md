...
トレーを構成箱ごとに分けて通過範囲を再計算すると、配線予約との検出は消えた。一方、電池とTTL基板の干渉は実形状でも確認された。後方16 mm移動時の重なりは約636.77 mm³。ベルトも後方14 mmで約61.06 mm³の重なりがある。したがって単なる外接箱の誤検出として無視できない。

`--remove-ttl` を明示し、TTL基板を先に取り外す整備条件では、後方82 mmまでの保守的な通過範囲に体積干渉なし。モジュール全体が胴体後端x=-64 mmより後ろへ抜ける。結果は `battery_service_without_ttl.json`、基板を残した失敗結果は `battery_service_path_refined.json`。工具・手の空間、基板を取り外す方法、配線とコネクターの切離しは未検証。この条件付き経路を、工具不要の電池交換や実物の整備完了とは扱わない。
