"""DC corner comparison of TPS259823 PG to TPS3700 INA+ receiver; not transient validation."""
import argparse
import hashlib
import itertools
import json
...
rows=[]
for supply,rp,rt,rb,ipg,ii in itertools.product([3.207,3.393],[148500,151500],[990000,1010000],[326700,333300],[0,1.7e-6],[-25e-9,25e-9]):
    # KCL solved exactly after eliminating divider node.
    pg=(supply/rp-ipg-ii*rb/(rt+rb))/(1/rp+1/(rt+rb))
    sense=(pg-ii*rt)*rb/(rt+rb)
    low_sense=(.786-ii*rt)*rb/(rt+rb)
