...
p=argparse.ArgumentParser();p.add_argument('--efuse-zip',type=Path,required=True);p.add_argument('--ldo-zip',type=Path,required=True);args=p.parse_args()
OUT=ROOT/'validation/pack_startup_pspice_v1';OUT.mkdir(exist_ok=True)
...
load=json.loads(budget_path.read_text())['rows'][-1]['partial_screen_sum_A'];resistance=3.3/load
...
report={'source_sha256':hashlib.sha256(source.read_bytes()).hexdigest(),'models':models,'executed':False,'qualification':False,'probe_load_ohm':resistance,'load_scope':'Partial budget at3.3V represented by resistor, not full MCU current model','budget_sha256':hashlib.sha256(budget_path.read_bytes()).hexdigest(),'source_ramp_s':.0001,'target_time_s':.05,'max_step_s':1e-6,'limits':['Nominal model only; no temperature or tolerance qualification','LDO model omits input-dependent quiescent/ground current','eFuse thermal timing is simplified, not actual temperature integration','No MCU state/reset/enable sequencing','No upstream PCM, fuse, reverse protection or contact model'],'model_files_redistributed':False}
