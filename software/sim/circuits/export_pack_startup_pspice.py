...
p=argparse.ArgumentParser();p.add_argument('--efuse-zip',type=Path,required=True);p.add_argument('--ldo-zip',type=Path,required=True);p.add_argument('--include-main-interface-load',action='store_true');args=p.parse_args()
OUT=ROOT/('validation/pack_startup_pspice_v2' if args.include_main_interface_load else 'validation/pack_startup_pspice_v1');OUT.mkdir(exist_ok=True)
source=ROOT/'schematics/power/pack_control_ramp_candidate_v1/assembly.json'
budget_path=ROOT/('validation/main_allow_interface_screen_v1/report.json' if args.include_main_interface_load else 'validation/pack_controller_budget_v1/report.json')
budget=json.loads(budget_path.read_text())
if args.include_main_interface_load:
 ...
 rows=budget['updated_partial_controller_budgets']
