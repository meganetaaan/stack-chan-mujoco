...
for case in range(102):
 ...
 def write(port,reg,value):
  ...
  if dropped_latch and reg==24:return
  regs[port,reg]=value
 ...
 if blocked:
  ...
  continue
 if dropped_latch:
  assert rc==0 and len(trace)==4
  assert (regs[0,0]>>16)&3==3  # PA8 remains analog.
  continue
 assert rc==1
...
r={'initial_register_patterns':100,'blocked_alias_disable_rejected':True,'dropped_latch_write_rejected_before_output_mode':True,'host_and_arm_compile':True,'first_trace':traces,
...
(a.out/'report.json').write_text(json.dumps(r,indent=2)+'\n');print('100 patterns and two failed-write cases passed')
