...
bq_pins.update({'21':'BQ_TS_CELL','23':'BQ_TS_FET','30':'BQ_BOTHOFF_N','29':'CELL_B_MINUS','33':'CELL_B_MINUS'})
...
assert len(parts)==54 and len({p['reference'] for p in parts})==54
...
assembly={'status':'partial_connection_candidate','parts':parts,'part_count':len(parts),'unresolved_BQ76942_pins':unresolved,'source_sha256':{s:hashlib.sha256((power/s).read_bytes()).hexdigest() for s in sources},'external_ports':['CELL_B_MINUS','CELL1_TAP','CELL2_TAP','CELL3_TAP','CELL_POS_FUSED','PACK_POS_PROTECTED','PACK_RETURN','BQ_ALLOW_BMINUS'],'missing_stages':['cell tap and main connectors','battery fuse and reverse protection','current threshold, shunt thermal/pulse and sense-routing qualification','BAT hold-up and input faults; REG18/CP1 effective capacitance and startup; optional LDO supply','PACK/LD transient and reverse-polarity qualification','configuration before allow; independent control supply and driver; cold-start and reset qualification','thermistor attachment, harness, fault response and temperature protection settings','system/servo integration'],'temperature_configuration_candidate':{'TS1':{'address':'0x92FD','value':'0x07','role':'cell temperature'},'TS3':{'address':'0x92FF','value':'0x0F','role':'FET temperature'},'source':'TI SLUUBY1B tables 13-9/13-11 and memory map','applied_to_hardware':False,'full_protection_configuration':False},'startup_inhibit_candidate':{'DFETOFF_config':{'address':'0x92FB','value':'0xC2','active_level':'low','function':'BOTHOFF'},'CFETOFF_config':{'address':'0x92FA','value':'0x00'},'RST_SHUT':'unused tied to VSS; not emergency stop','effective_before_configuration':False,'external_allow_driver_qualified':False,'control_reference':'CELL_B_MINUS'},'electrically_operational':False,'manufacturing_release':False}
