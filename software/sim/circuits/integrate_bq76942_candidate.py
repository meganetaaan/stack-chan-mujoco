...
sources=['bq76942_candidate.json','bq76942_series_filter_candidate.json','bq76942_pack_fet_candidate.json','bq76942_reg0_limiter_candidate.json','bq_local_host_candidate.json','bq_host_watchdog_candidate.json']
a,c,q,limiter,host,watchdog=[json.loads((power/s).read_text()) for s in sources]
parts=[]
...
parts.extend(watchdog['parts'])
assert len(parts)==100 and len({p['reference'] for p in parts})==100
