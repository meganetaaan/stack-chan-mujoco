...
files={'base':'schematics/power/servo_power_pololu_integration_candidate_v1/assembly.json','resistor':'schematics/power/passive_discharge_candidate.json','old_plan':'validation/passive_discharge_candidate_v2/plan.json','inhibit':'schematics/power/pololu_startup_inhibit_candidate.json'}
...
# Add local default-low ENA networks; the original base is retained as history.
for side in ['LEFT','RIGHT']:
 module=next(x for x in out['parts'] if x['reference']==side+'_U_REGULATOR')
 module['pins']['ENA']=side+'_REGULATOR_ENA'
 out['parts'].append({'reference':side+'_R_REG_EN_PD','part':d['inhibit']['external_pulldown']['part'],'pins':{'1':side+'_REGULATOR_ENA','2':'GND'}})
 out['parts'].append({'reference':side+'_R_REG_EN_SER','part':d['inhibit']['external_series']['part'],'pins':{'1':side+'_REGULATOR_ALLOW','2':side+'_REGULATOR_ENA'}})
refs=[p['reference'] for p in out['parts']];assert len(refs)==len(set(refs));byref={p['reference']:p for p in out['parts']}
for side in ['LEFT','RIGHT']:
...
out['candidate_integration']={'inputs':files,'source_sha256':{k:hashlib.sha256(Path(v).read_bytes()).hexdigest() for k,v in files.items()},'base_parts_unchanged':False,'base_changes':['LEFT_U_REGULATOR ENA','RIGHT_U_REGULATOR ENA'],'current_pointer_changed':False,'previous_candidate_integration':base.get('candidate_integration')}
report={'part_count':len(refs),'connections':out['pin_count'],'added_resistors':8,'regulator_ENA_default_low_candidate':True,'ENA_threshold_and_host_qualified':False,'downstream_of_reverse_FET':True,'left_right_positives_not_connected':True,'rows':rows,'comparison_max_current_per_leg_at5_25V_A':2*5.25/rmin,'comparison_max_total_power_at5_25V_W':4*5.25**2/rmin,'each_resistor_power_at6V_W':6**2/rmin,'capacitance_and_5percent_resistance_are_unqualified_assumptions':True,'bleed_is_not_regenerative_brake':True,'manufacturing_release':False}
