...
c=json.loads((root/'schematics/power/manual_rearm_current.json').read_text());asm=json.loads((root/c['assembly']).read_text());bom={ref:r for r in csv.DictReader((root/c['quantity_bom']).open()) for ref in r['references'].split()}
...
for index,part in enumerate(asm['parts']):
 ...
 for j,pin in enumerate(pins):
  local_y=(n-1)*1.27-j*2.54;px=x-15.24;py=y-local_y;net=part['pins'][pin]
...
text=f'(kicad_sch (version 20230121) (generator eeschema) (uuid {sheet}) (paper "A2") (lib_symbols {" ".join(libs)}) (text "CONNECTION MIGRATION ONLY - generic passive pins; no footprints; not for fabrication" (at 297 15 0) {effects(1.5)} (uuid {uid("warning")})) {" ".join(connections)} {" ".join(instances)})\n'
