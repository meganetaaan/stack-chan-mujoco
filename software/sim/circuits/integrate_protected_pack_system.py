...
parser=argparse.ArgumentParser();parser.add_argument('--connect-aux',action='store_true');args=parser.parse_args()
OUT=ROOT/('schematics/power/protected_pack_system_candidate_v2' if args.connect_aux else 'schematics/power/protected_pack_system_candidate_v1');OUT.mkdir(exist_ok=True)
...
if args.connect_aux:
 check=ROOT/'validation/aux_start_connection_v1/report.json';review=json.loads(check.read_text())
 original=ROOT/'schematics/power/protected_pack_system_candidate_v1/assembly.json'
 assert review['source_sha256']==hashlib.sha256(original.read_bytes()).hexdigest() and all(review['checks'].values())
 files.append(check);bindings['LOGIC_START_ALLOW']='SYS__LOGIC_START_ALLOW'
parts=[]
...
assert ('LOGIC_START_ALLOW' not in nets if args.connect_aux else 'LOGIC_START_ALLOW' in nets) and 'SYS__LOGIC_START_ALLOW' in nets
if args.connect_aux:assert refs['CTRL__U_AUX_GATE']['pins']['4']==refs['SYS__R_LOGIC_SHDN_SER']['pins']['1']
