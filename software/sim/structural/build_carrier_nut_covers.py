...
for sign in [-1,1]:
 for z in [88,112]:
  ...
  cap=box([13.6,1,6.4],[45,sign*54.5,z])
...
result={'carrier_valid':carrier.isValid(),'carrier_solids':len(carrier.Solids()),'external_overlaps':hits,'cover_carrier_overlaps':internal,'nut_escape_checks':escape,'source_sha256':{str(f.relative_to(ROOT)):hashlib.sha256(f.read_bytes()).hexdigest() for f in [p/'report.json',p/'assembly.step']},'manufacturing_release':False,'limits':['Zero nominal sliding gap requires tolerance redesign','No cap insertion or all-direction capture proof','Rails and cover strength unqualified']}
