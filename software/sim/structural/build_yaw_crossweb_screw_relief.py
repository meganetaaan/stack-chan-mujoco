"""Front crossweb with screw relief in added material only, preserving the existing bounding envelope."""
import argparse,hashlib,json
from pathlib import Path
import cadquery as cq
p=argparse.ArgumentParser(description=__doc__);p.add_argument('--out',type=Path,required=True);a=p.parse_args();a.out.mkdir(parents=True,exist_ok=False)
plan={'question':'Can a front transverse web tie the front shelf edge without enlarging the envelope or blocking fixed hardware?',
 'candidate':'Web X8..11.6, Y=cy +/-18, Z78..88 mm; connects the existing deep side ribs below the plate seat.',
 'relief': 'Remove radius 2.7 mm vertical columns around plate screws 2/3 from added material only; retain original support',
 'assumptions':{'web_thickness_mm':3.6,'web_drop_below_shelf_mm':10,'material':'printed ABS/PETG, grade and printing not qualified'},
...
for side,cy in [('left',26),('right',-26)]:
 ...
 old=cq.importers.importStep(str(path)).val();web=cq.Solid.makeBox(3.6,36,10,cq.Vector(8,cy-18,78));raw=old.fuse(web).clean();added=raw.cut(old)
