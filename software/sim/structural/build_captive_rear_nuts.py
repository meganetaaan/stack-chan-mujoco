"""Square-nut retention candidates; no strength or assembly release."""
import argparse,hashlib,json,math
from pathlib import Path
import cadquery as cq
ROOT=Path(__file__).resolve().parents[3]
p=argparse.ArgumentParser(description=__doc__);p.add_argument('--out',type=Path,required=True);p.add_argument('--slot-width-mm',type=float,default=6.4);p.add_argument('--loading-mode',choices=['side','embedded'],default='side')
a=p.parse_args()
if not 6.2<=a.slot_width_mm<=6.6:p.error('Slot width must be 6.2 to 6.6 mm')
...
plan={'scope':__doc__,'loading_mode':a.loading_mode,'nut':{'candidate':'Accu HFSN-M3-A4, DIN562','source':'https://www.accu.co.uk/flat-square-nuts/21333-HFSN-M3-A4','width_max_mm':5.5,'width_min_mm':5.1,'thickness_max_mm':1.8,'thickness_min_mm':1.4,'thread':'M3x0.5','strength_class':'not qualified'},
      'pocket':{'width_z_mm':a.slot_width_mm,'thickness_x_mm':(2.9 if a.loading_mode=='embedded' else 2.4),'surface_tolerance_mm':.2,'nut_back_x_mm':-55.7,'slot_back_x_mm':(-56.2 if a.loading_mode=='embedded' else -55.9),'slot_front_x_mm':(-53.3 if a.loading_mode=='embedded' else -53.5)},
      ...
      'limitations':['square envelope ignores corner radii','nut thread strength/preload not verified','retention under tightening torque not verified','insertion starts inside body; human/tool approach not yet verified',('side-open pocket does not prevent nut escape before bolt insertion' if a.loading_mode=='side' else 'embedded hardware requires pause-and-insert manufacturing; replacement may require a new body')]}
if a.loading_mode=='embedded':
 plan['inner_washer']={'candidate':'Accu HRDW-M3-A4 DIN433','source':'https://www.accu.co.uk/metric-flat-washers/404911-HRDW-M3-A4','OD_mm':[5.7,6.0],'ID_mm':[3.2,3.38],'thickness_mm':[.45,.55]}
...
for y,z,sign in centers:
    ...
    if a.loading_mode=='side':body=body.cut(cyl(3.8,.7,-56.3,y,z))
    # Slot opens toward the body center; outer end retains nut positioning.
    slot_y=(y-20 if sign>0 else y-3.1) if a.loading_mode=='side' else y-a.slot_width_mm/2
    slot_length=23.1 if a.loading_mode=='side' else a.slot_width_mm
    body=body.cut(box(plan['pocket']['slot_back_x_mm'],slot_y,z-a.slot_width_mm/2,plan['pocket']['thickness_x_mm'],slot_length,a.slot_width_mm))
...
for i,(y,z,sign) in enumerate(centers):
    ...
    washer=cyl(3. if a.loading_mode=='embedded' else 3.5,.5,-56.2,y,z).cut(cyl(1.6,.5,-56.2,y,z))
    shapes=[('nut',nut),('bolt',bolt),('inner_washer',washer)]
    if a.loading_mode=='side':shapes.append(('insertion_sweep',sweep))
...
width_min=a.slot_width_mm-.4;width_max=a.slot_width_mm+.4;thickness_min=plan['pocket']['thickness_x_mm']-.4
