...
for name,E,nu,patch,loadpatch,sign in [('boss',1120,.35,'spacer_contact','nut_bearing',-1),('spacer',193000,.3,'boss_contact','head_bearing',1)]:
 ...
 parts.append({'name':name,'mesh':mesh,'basis':basis,'xyz':xyz,'lam':lam,'mu':mu,'contactw':contactw,'offset':offset,'sha256':hashlib.sha256(path.read_bytes()).hexdigest()});Ks.append(K);Cs.append(csr_matrix(C));Fs.append(F);offset+=basis.N
...
initial_gap=np.zeros(len(area));contact_coordinates=boss['xyz'][ids]
if a.projected_contact or a.clipped_contact:
 B,area,initial_gap,contact_coordinates,projection_report=project(boss['mesh'],spacer['mesh']);(a.out/'projection.json').write_text(json.dumps(projection_report,indent=2)+'\n')
solution=solve(K,C,F,B,area,plan['penalty_N_mm3'],initial_gap=initial_gap,max_iterations=plan['max_iterations']);u=solution['u'];multipliers=solution['multipliers'];gap=solution['gap'];history=solution['history'];converged=solution['converged'];A=solution['matrix']
clipped_info=None
if a.clipped_contact:
 patches,ndof,prep=prepare(boss['mesh'],spacer['mesh']);assert ndof==len(F)
 u,multipliers,clipped_force,clipped_info,history,converged,A=equilibrate(K,C,F,patches,u,multipliers,plan['penalty_N_mm3'])
 gap=initial_gap+B@u
 (a.out/'clipped_integration.json').write_text(json.dumps({'preparation':prep,'integration':clipped_info,'history':history,'converged':converged},indent=2)+'\n')
pressure=np.maximum(-plan['penalty_N_mm3']*gap,0);contact_force=clipped_force if a.clipped_contact else B.T@(area*pressure);gauge_force=-C.T@multipliers;residual=K@u-F-contact_force-gauge_force;results={};arrays={'u_mm':u,'pressure_MPa':pressure,'gap_mm':gap,'contact_area_mm2':area,'gauge_force_N':gauge_force,'multipliers':multipliers,'contact_coordinates_mm':contact_coordinates,'initial_gap_mm':initial_gap,'contact_force_N':contact_force}
...
ratio=float(np.linalg.norm(gauge_force.reshape(-1,3),axis=1).sum()/20);eq=float(np.linalg.norm(residual));cr=float(abs(C@u).max());penetration=clipped_info['peak_pressure_MPa']/plan['penalty_N_mm3'] if a.clipped_contact else float(max(0,-gap.min()));r={'converged_active_set':converged,'iterations':history,'elapsed_seconds':time.monotonic()-start,'matrix_dofs':A.shape[0],'matrix_nonzeros':A.nnz,'equilibrium_residual_N':eq,'mean_constraint_residual':cr,'gauge_force_sum_relative':ratio,'contact_total_N':float(contact_force[:spacer['offset']].reshape(-1,3)[:,2].sum()),'maximum_penetration_mm':penetration,'maximum_pressure_MPa':clipped_info['peak_pressure_MPa'] if a.clipped_contact else float(pressure.max()),'parts':results,'gates':{'active_set':converged,'equilibrium':eq<=1e-7,'mean_constraints':cr<=1e-8,'penetration':penetration<=.01,'gauge_force':ratio<=1e-4},'joint_verified':False,'source_sha256':{x['name']:x['sha256'] for x in parts}};(a.out/'report.json').write_text(json.dumps(r,indent=2)+'\n');np.savez_compressed(a.out/'fields.npz',**arrays);print(json.dumps(r,indent=2))
