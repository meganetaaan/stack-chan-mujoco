...
for side,cy in [('left',26),('right',-26)]:
 ...
 for z in [64,76]:
  ...
  support=support.cut(cyl(1.15,9,-62.3)).cut(cyl(2.1,1.4,-62.3));bar=bar.cut(cyl(.8,3,-53.5))
  head=cyl(1.9,1.3,-62.2);shaft=cyl(1,10,-60.9);screw=head.fuse(shaft)
  rows.append({'side':side,'z_mm':z,'nominal_thread_entry_mm':2.6,'nominal_tip_recess_mm':.4,'head_rear_face_x_mm':-62.2,'screw_rear_plate_overlap_mm3':screw.intersect(rear).Volume(),'screw_support_overlap_mm3':screw.intersect(support).Volume()})
