...
cp=Path('validation/direct_cell_p30b_v1/cell_candidate.json');spec=json.loads(cp.read_text());root=Path('validation/yaw_integrated_candidate_v4');ip=root/'inventory.json';sp=root/'yaw_support_candidate.step'
...
report={'rows':rows,'tab5_source_sha256':hashlib.sha256(tabpath.read_bytes()).hexdigest(),'overlapping_parts':[r['part'] for r in rows if r['overlap_mm3']>1e-6],'minimum_nominal_distance_mm':min(r['distance_mm'] for r in rows),'cell_only_mass_max_g':spec['mass_max_g'],'pack_and_holder_fit_verified':False,'manufacturing_release':False}
