...
from rigid_payload_inertia import angular_motion, couple_on_support, verify_analytic_cases

ROOT = Path(__file__).resolve().parents[3]
OUT = ROOT / 'validation/tab5_carrier_load_map_v2'
...
plan = {
    ...
    'exclusions': ['Insert and Tab5 fastening hardware mass', 'Tab5 COM measurement',
                   'Preload and friction/contact separation',
                   'Updated robot dynamics, fall and emergency stop'],
    'tab5_inertia_assumption': 'Uniform 12 x 128 x 80 mm cuboid, not measured inertia',
...
tab_dims = np.array([.012, .128, .080])
tab_inertia = np.diag(.1184/12*(np.sum(tab_dims**2)-tab_dims**2))
carrier_inertia = np.array(cq.Shape.matrixOfInertia(carrier))*1270*1e-15
inertia = tab_inertia + carrier_inertia
analytic_errors = verify_analytic_cases()
rows=[]
paths=[carrier_path, Path(__file__), Path(__file__).with_name('rigid_payload_inertia.py'), Path(__file__).with_name('estimate_battery_translation_load.py')]
for path in sorted((ROOT/'validation/prototype_epic4_v1').glob('actuator_fullbody_load_*/trace.npz')):
    ...
    transfer_moment = moment.copy()
    omega, alpha = angular_motion(t,q[:,3:7])
    inertial_couple = couple_on_support(omega,alpha,inertia)
    moment += inertial_couple
    ...
    np.savez_compressed(OUT/(path.parent.name+'.npz'), time_s=t,
                        applied_force_N=force,force_transfer_moment_Nm=transfer_moment,
                        angular_inertial_couple_Nm=inertial_couple, total_moment_Nm=moment,
                        node_force_N=loads,valid_indices=valid)
    rows.append({'time_first_s':float(t[0]),'time_last_s':float(t[-1]),
                 'max_angular_couple_norm_Nm':float(np.linalg.norm(inertial_couple[valid],axis=1).max()),
                 'source':str(path.relative_to(ROOT)),'equilibrium_residual':residual,'selected':selected})
...
result={'status':'conditional_force_transfer_not_design_upper_bound',
        ...
        'angular_analytic_case_errors_Nm':analytic_errors,
        'sum_com_inertias_base_kg_m2':inertia.tolist(),
