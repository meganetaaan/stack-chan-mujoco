...
p=argparse.ArgumentParser(description=__doc__);p.add_argument('--body',type=Path,required=True);p.add_argument('--out',type=Path,required=True);p.add_argument('--path-mode',choices=['direct','lift20','lift14','lift9'],default='lift9');p.add_argument('--tool-profile',choices=['slender','stiff6'],default='stiff6');p.add_argument('--fastener-check',choices=['on','off'],default='on')
a=p.parse_args();lateral={'direct':20,'lift20':20,'lift14':14,'lift9':9}[a.path_mode];lower_lift=0 if a.path_mode=='direct' else 52
...
plan={'scope':__doc__,'stage':'bare body and rear plate, before Tab5, yaw supports, battery and power installation',
      ...
      'tool_profile':a.tool_profile,'shaft_section_mm':([6,6] if a.tool_profile=='stiff6' else [2,2]),'shaft_end_x_mm':80,'handle_end_x_mm':125,
...
for sign in [-1,1]:
 for z in [8,120]:
    ...
    if a.tool_profile=='stiff6':
        boxes['stem']=[-53.9,-47.9,44,50,z-3,z+lift+3]
        boxes['shaft']=[-47.9,80,44,50,z+lift-3,z+lift+3]
        boxes['handle']=[80,125,42,52,z+lift-5,z+lift+5]
    core=box_bounds(boxes['hub']).fuse(*[box_bounds(boxes[k]) for k in ['stem','shaft'] if k in boxes]).clean()
    assert core.isValid() and len(core.Solids())==1
    if sign<0:core=core.mirror('XZ')
    cq.exporters.export(core,str(a.out/f'{sign}_{z}_core.step'))
