...
ROOT=Path(__file__).resolve().parents[3];out=ROOT/'validation/lb020_retention_v5';out.mkdir(exist_ok=True)
plan={'strap_centers_y_mm':[-20,20],'strap_width_mm':12.7,'strap_thickness_envelope_mm':2.5,'closure_layers':2,'closure_region':'top z>=98 or front x>=43.2 and z>=65','guide_clear_width_mm':13.7,'candidate_part':'VELCRO 31013','cut_length_mm':200,'basis':'Manufacturer width12.7mm;2.5mm single-layer thickness is an unverified design allowance. Place and constrain closure on top/front route; rear side remains one layer.2.5mm thickness allowance unchanged; this is a different specified assembly, not a relaxation of v2.','criteria':['Valid single tray solid','No volume overlap with battery or straps','No new volume overlap with98 torso parts'],'threshold_mm3':.01,'stop':'One two-strap route; do not perform FE or claim retention before selecting closure and load/pressure limits'}
...
for y in plan['strap_centers_y_mm']:
 ...
 closure_region=box([50,15,10],[29,y,103]).fuse(box([10,15,45],[48.2,y,87.5]))
 upper=double.intersect(closure_region)
...
result={'valid_tray':tray.isValid(),'tray_solids':len(tray.Solids()),'volume_mm3':tray.Volume(),'internal_overlaps_mm3':internal,'external_overlaps':checks,'strap_min_external_gap_mm':{n:min(s.distance(t) for m,t in parts.items() if m not in ('tray','battery')) for n,s in objects.items() if n.startswith('strap')},'source_sha256':{str(f.relative_to(ROOT)):hashlib.sha256(f.read_bytes()).hexdigest() for f in [source,source.with_name('battery.step'),p/'report.json',p/'torso_candidate.step']},'manufacturing_release':False,'limits':['Top/front double-layer reservation; closure must not migrate to rear; bend radius and installation unqualified','Published closure values are typical, not design allowables; pressure limit unknown','No tolerance, flexure or strength qualification']}
