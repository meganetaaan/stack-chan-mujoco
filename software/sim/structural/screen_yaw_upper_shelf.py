"""Bounded deformation check of upper shelf reinforcement under the largest directly evaluated archive case."""
import argparse,hashlib,json
from pathlib import Path
from elasticity import tetrahedralize,analyze
p=argparse.ArgumentParser(description=__doc__);p.add_argument('--out',type=Path,required=True);p.add_argument('--step',type=Path,default=Path('validation/yaw_upper_shelf_v1/left_yaw_fixed_support.step'));a=p.parse_args();a.out.mkdir(parents=True,exist_ok=False)
source=Path('validation/yaw_current_archive_envelope_v1/report.json');archive=json.loads(source.read_text());case=max(archive['rows'],key=lambda r:r['selected_actual_displacement_mm']);old={'origin_mm':[-5,26,62],'source_case':case};step=a.step
