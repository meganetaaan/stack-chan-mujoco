"""One inward-thickened side-rib candidate, preserving the existing bounding envelope."""
...
for side,cy in [('left',26),('right',-26)]:
 ...
 old=cq.importers.importStep(str(path)).val();webs=[cq.Solid.makeBox(40,6.5,18,cq.Vector(-30,cy+dy,70)) for dy in [-18,11.5]];new=old.fuse(*webs).clean();added=new.cut(old)
