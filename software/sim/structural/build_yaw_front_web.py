"""One front cross-web candidate, preserving the existing bounding envelope."""
...
plan={'question':'Can a front transverse web tie the front shelf edge without enlarging the envelope or blocking fixed hardware?',
 'candidate':'Web X12..13.6, Y=cy +/-18, Z78..89 mm. Upper 1 mm joins existing shelf outside plate pocket.',
 'assumptions':{'web_thickness_mm':1.6,'web_drop_below_shelf_mm':10,'material':'printed ABS/PETG, grade and printing not qualified'},
...
for side,cy in [('left',26),('right',-26)]:
 ...
 old=cq.importers.importStep(str(path)).val();web=cq.Solid.makeBox(1.6,36,11,cq.Vector(12,cy-18,78));new=old.fuse(web).clean();added=new.cut(old)
