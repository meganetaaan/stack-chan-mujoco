...
ROOT=Path(__file__).resolve().parents[3];OUT=ROOT/'validation/tab5_frame_screws_v1';OUT.mkdir(exist_ok=True)
...
plan={'candidate':'NBK SLH-M3-8 x4','head_diameter_mm':5.5,'head_height_mm':2,'length_mm':8,'seat_x_mm':48,'Tab5_nominal_back_x_mm':52,'nominal_penetration_mm':4,'criteria':'Flag interference above0.01mm3 against other nominal parts; Tab5 thread envelope recorded separately, not pass','stop':'One8mm candidate; no blind-depth inference from mesh or M3*10 annotation'}
(OUT/'plan.json').write_text(json.dumps(plan,indent=2)+'\n')
rows=[];screws=[]
for y in [-60,60]:
 for z in [52,124]:
  shaft=cq.Solid.makeCylinder(1.5,8,cq.Vector(48,y,z),cq.Vector(1,0,0));head=cq.Solid.makeCylinder(2.75,2,cq.Vector(46,y,z),cq.Vector(1,0,0));screw=shaft.fuse(head)
...
result={'rows':rows,'all_other_parts_clear':all(not row['unexpected_overlaps'] for row in rows),
 'candidate_added_mass_g':4*.62,'thread_engagement_qualified':False,'mount_torque_qualified':False,
 'screw_length_condition':'Usable female depth >=4mm plus tip/bottom/tolerance allowance; minimum effective thread engagement must also be specified',
