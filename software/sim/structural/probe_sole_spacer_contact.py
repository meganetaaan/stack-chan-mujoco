...
p=argparse.ArgumentParser(description=__doc__);p.add_argument('--boss-mesh-dir',type=Path);p.add_argument('--spacer-mesh-dir',type=Path);p.add_argument('--reverse-contact',action='store_true');p.add_argument('--out',type=Path,required=True);p.add_argument('--mesh-mm',default='0.7');a=p.parse_args()
...
lines+=['*NODE PRINT,NSET=ANCHORS,TOTALS=ONLY','RF','*NODE PRINT,NSET=BOSS','U','*NODE PRINT,NSET=SPACER','U',('*CONTACT PRINT,SLAVE=SPACER_CONTACT,MASTER=BOSS_CONTACT' if a.reverse_contact else '*CONTACT PRINT,SLAVE=BOSS_CONTACT,MASTER=SPACER_CONTACT'),'CF,CDIS,CSTR','*EL PRINT,ELSET=BOSS','S','*EL PRINT,ELSET=SPACER','S','*END STEP']
