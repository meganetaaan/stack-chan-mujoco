...
if __name__=='__main__':
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--mesh-dir',type=Path,default=Path('validation/sole_extended_contact_mesh_v1'));p.add_argument('--source',type=Path,default=Path('validation/sole_projected_contact_probe_v1'));p.add_argument('--mesh-mm',default='0.5');p.add_argument('--out',type=Path,required=True);a=p.parse_args();a.out.mkdir(parents=True,exist_ok=False)
    plan={'scope':__doc__,'mesh_dir':str(a.mesh_dir),'source':str(a.source),'mesh_mm':a.mesh_mm,'source_sha256':{str(f):hashlib.sha256(f.read_bytes()).hexdigest() for f in [a.source/'fields.npz']+[a.mesh_dir/n/f'{n}_{a.mesh_mm}.msh' for n in ['boss','spacer']]},'relative_area_coverage_tolerance':1e-8,'maximum_penetration_mm':.01,'assumptions':['P1 displacement on initial planar triangles, vertical projection and small-displacement gap.','Finite rotation, curved CAD between facets and actual surface roughness excluded.'],'clipping_halfplane_tolerance_mm2':1e-12};(a.out/'plan.json').write_text(json.dumps(plan,indent=2)+'\n')
    root=a.mesh_dir;b,s=[from_meshio(meshio.read(root/n/f'{n}_{a.mesh_mm}.msh')) for n in ['boss','spacer']];bx=b.p.T;sx=s.p.T
    bt=b.facets[:,np.r_[b.boundaries['spacer_contact'],b.boundaries['contact_extension']]].T;st=s.facets[:,np.r_[s.boundaries['boss_contact'],s.boundaries['contact_extension']]].T;master=bx[bt];lower=master[:,:,:2].min(axis=1);upper=master[:,:,:2].max(axis=1);u=np.load(a.source/'fields.npz')['u_mm'];bu=u[:3*len(bx)].reshape(-1,3);su=u[3*len(bx):].reshape(-1,3)
    ...
    for i,ids in enumerate(st):
     ...
     for j in candidates:
      ...
      for q in poly:
       ...
       if gap<minimum:minimum=float(gap);peak={'xy_mm':np.asarray(q).tolist(),'slave_triangle_index':i,'master_triangle_index':int(j),'initial_gap_mm':float(bw@master[j,:,2]-sw@tri[:,2]),'boss_vertical_displacement_mm':float(bw@bu[bt[j],2]),'spacer_vertical_displacement_mm':float(sw@su[ids,2]),'boss_triangle_xyz_mm':master[j].tolist(),'spacer_triangle_xyz_mm':tri.tolist()}
