...
plan={'question':'Can a thicker front shelf reduce local bending without enlarging the envelope or blocking fixed hardware?',
 ...
 'relief': 'Remove radius 3.3 mm vertical columns around all four plate screws from added material only; retain original support',
 'assumptions':{'shelf_extra_thickness_mm':2,'material':'printed ABS/PETG, grade and printing not qualified'},
...
for side,cy in [('left',26),('right',-26)]:
 ...
 for item,solid in zip(parts,members):
  ...
  if item['name'] in [side+f'_plate_{i}_screw' for i in range(4)]:
   b=solid.BoundingBox();reliefs.append(cq.Solid.makeCylinder(3.3,30,cq.Vector((b.xmin+b.xmax)/2,(b.ymin+b.ymax)/2,65)))
