"""Extend the existing shelf profile upward within the assembly envelope."""
...
plan={'question':'Can a thicker front shelf reduce local bending without enlarging the envelope or blocking fixed hardware?',
 'candidate':'Translate existing shelf slice Z89..91 upward 2 mm, preserving its openings; retain metal servo seat.',
 'relief': 'Remove radius 4.0 mm vertical columns around all four plate screws from added material only; retain original support',
 'assumptions':{'shelf_extra_thickness_mm':2,'tool_relief_radius_mm':4.0,'tool_candidate':'Wera 05118120001, catalog blade diameter5.7 mm','nominal_radial_tool_gap_mm':1.15,'material':'printed ABS/PETG, grade and printing not qualified'},
 'criteria':['valid single solid','unchanged whole assembly bounding box','added-material overlap with fixed assembly and manufacturer case/connectors <=0.01 mm3','reserved outer-port corridor distance >=1.3 mm'],
...
for side,cy in [('left',26),('right',-26)]:
 ...
 old=cq.importers.importStep(str(path)).val();web=old.intersect(cq.Solid.makeBox(100,36,2,cq.Vector(-65,cy-18,89))).translate((0,0,2));raw=old.fuse(web).clean();added=raw.cut(old)
 ...
 for item,solid in zip(parts,members):
  ...
  if item['name'] in [side+f'_plate_{i}_screw' for i in range(4)]:
   b=solid.BoundingBox();reliefs.append(cq.Solid.makeCylinder(4.0,30,cq.Vector((b.xmin+b.xmax)/2,(b.ymin+b.ymax)/2,65)))
 assert len(reliefs)==4
 added=added.cut(*reliefs).clean();new=old.fuse(added).clean();assert old.cut(new).Volume()<1e-6
 before=fixed.BoundingBox();after=cq.Compound.makeCompound([fixed,new]).BoundingBox();delta=max(abs(getattr(before,k)-getattr(after,k)) for k in ['xmin','xmax','ymin','ymax','zmin','zmax'])
 collisions=[]
 members=fixed.Solids();assert len(members)==len(parts)
 for item,solid in zip(parts,members):
  assert abs(solid.Volume()-item['volume_mm3'])<1e-5
