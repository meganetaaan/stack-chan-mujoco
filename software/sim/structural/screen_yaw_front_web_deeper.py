"""Bounded deformation check of deeper front web under the largest directly evaluated archive case."""
import argparse,hashlib,json
from pathlib import Path
from elasticity import tetrahedralize,analyze
p=argparse.ArgumentParser(description=__doc__);p.add_argument('--out',type=Path,required=True);p.add_argument('--step',type=Path,default=Path('validation/yaw_front_web_deeper_v1/left_yaw_fixed_support.step'));a=p.parse_args();a.out.mkdir(parents=True,exist_ok=False)
