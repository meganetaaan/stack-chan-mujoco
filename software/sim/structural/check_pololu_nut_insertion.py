...
mounted={}
if 'mounted_assembly' in plan:
 root=Path(plan['mounted_assembly']);mi=read(root/'inventory.json')['parts'];mp=root/'mount_assembly.step';paths.append(mp);ms=cq.importers.importStep(str(mp)).val().Solids();assert len(mi)==len(ms)==43
 for item,solid in zip(mi,ms):
  assert abs(item['volume_mm3']-solid.Volume())<1e-5
  b=solid.BoundingBox();actual=[getattr(b,k) for k in ['xmin','xmax','ymin','ymax','zmin','zmax']]
  assert max(abs(x-y) for x,y in zip(actual,item['bounds_mm']))<1e-5
  if item['group'] in ['structure','rear_hardware'] or (item['group']=='module_hardware' and item['name'].endswith('_nut')):mounted[item['name']]=solid
 targets.update(mounted)
rows=[]
for place in places:
 side=place['side'];bp=Path('validation/pololu_captive_nut_v1')/(side+'_bracket.step');paths.append(bp);bracket=mounted.get(side+'_bracket') if mounted else cq.importers.importStep(str(bp)).val();mid=sum({v[1] for v in place['mount_axes_mm']})/2
 for i,(x,y,z) in enumerate(place['mount_axes_mm']):
  ...
  checks=[{'part':n,'overlap_mm3':swept.intersect(t).Volume(),'distance_mm':swept.distance(t)} for n,t in targets.items() if n not in [side+'_bracket',f'{side}_module_{i}_nut']]
