...
p=argparse.ArgumentParser(description=__doc__);p.add_argument('--support-mode',choices=['clamped','normal_z'],default='clamped');p.add_argument('--out',type=Path,required=True);a=p.parse_args()
...
plan={'scope':__doc__,'source_sha256':hashlib.sha256(source.read_bytes()).hexdigest(),'crop_mm':{'x':[-38,-30],'y':[-18.5,-10.5],'z':[-16,-12.8]},'mesh_mm':[1,.7,.5],'load_N':20,'young_MPa':1120,'poisson':.35,'support_mode':a.support_mode,'fixed':'Pad bottom z=-16; selected bilateral support plus minimal in-plane anchors for normal_z','load':'Entire cavity floor z=-14.6, downward 20 N; larger than actual nut bearing area','criteria':{'displacement_mm':.2,'stress_MPa':5.6,'last_displacement_relative_change':.05,'last_stress_relative_change':.1},'limitations':['Single left rear local submodel, not whole boot or full joint.','Cut boundaries free; omitted surrounding material changes stiffness.','Load smeared over cavity floor, no contact/preload solution.','normal_z is bilateral, permits artificial tensile reactions and does not model separation.','Isotropic PETG constants and allowable are unqualified assumptions; no creep/buckling.']}
...
for size in plan['mesh_mm']:
 ...
 report,data=analyze(mesh,lambda x:abs(x[2]+16)<1e-6,lambda x:abs(x[2]+14.6)<1e-6,[-34,-14.5,-14.6],[0,0,-20,0,0,0],1120,.35,support_mode=a.support_mode)
