...
p=argparse.ArgumentParser(description=__doc__);p.add_argument('--out',type=Path,required=True);p.add_argument('--outside-port-only',action='store_true');p.add_argument('--plate-dir',type=Path,default=Path('validation/yaw_connector_plate_relief_v1'));a=p.parse_args();a.out.mkdir(parents=True,exist_ok=False)
...
if a.outside_port_only: plan['centres_xy_mm']={'left':[[-14,34]],'right':[[-14,-34]]}
(a.out/'plan.json').write_text(json.dumps(plan,indent=2)+'\n');rows=[];hashes={}
for side,cy in [('left',26),('right',-26)]:
 sp=Path(f'validation/yaw_backing_keeper_v2/{side}_yaw_fixed_support.step');pp=a.plate_dir/f'{side}_mount_plate.step'
