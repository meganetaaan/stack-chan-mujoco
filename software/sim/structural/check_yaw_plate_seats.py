...
ROOT=Path(__file__).resolve().parents[3];p=argparse.ArgumentParser(description=__doc__);p.add_argument('--out',type=Path,required=True);p.add_argument('--relocated',action='store_true');p.add_argument('--candidate-version',choices=['v3','v4'],default='v3');p.add_argument('--front-axis-mm',type=float,default=7.5);a=p.parse_args();a.out.mkdir(parents=True,exist_ok=False)
...
version=a.candidate_version if a.relocated else 'v2'
for side,cy in [('left',26),('right',-26)]:
 parts={}
 for n in ['mount_plate','yaw_fixed_support']:
  path=ROOT/f'validation/yaw_metal_seat_{version}/{side}_{n}.step';sources[str(path.relative_to(ROOT))]=hashlib.sha256(path.read_bytes()).hexdigest();parts[n]=cq.importers.importStep(str(path)).val()
 for x in ([-34,a.front_axis_mm] if a.relocated else [-31.5,6.5]):
  for y in ([cy-10,cy+10] if a.relocated else [cy-13,cy+13]):
