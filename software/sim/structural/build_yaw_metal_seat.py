...
ROOT=Path(__file__).resolve().parents[3];p=argparse.ArgumentParser(description=__doc__);p.add_argument('--out',type=Path,required=True);p.add_argument('--wide-plate',action='store_true');p.add_argument('--relocated-axes',action='store_true');a=p.parse_args();a.out.mkdir(parents=True,exist_ok=False)
width,depth,x0=(48.5,33,-37.5) if a.relocated_axes else ((45,33,-35) if a.wide_plate else (42,30,-33.5))
...
for side,cy in [('left',26),('right',-26)]:
 ...
 if a.relocated_axes:support=support.fuse(cq.Solid.makeBox(1.01,36,3,cq.Vector(9.99,cy-18,88)))
 ...
 mounting_axes=[(x,y) for x in ([-34,7.5] if a.relocated_axes else [-31.5,6.5]) for y in ([cy-10,cy+10] if a.relocated_axes else [cy-13,cy+13])]
