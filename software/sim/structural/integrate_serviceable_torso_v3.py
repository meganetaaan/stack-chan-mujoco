...
ROOT=Path(__file__).resolve().parents[3];out=ROOT/'validation/serviceable_torso_v3';out.mkdir(exist_ok=True)
plan={'scope':'Static integration of current carrier hardware and LB-020 route candidates','criteria':'Flag all new-vs-retained and new-vs-new volume overlaps above0.01mm3','stop':'One integration audit; expected threaded screw/nut contact must be reviewed explicitly','limits':['Carrier installed envelope subtracts nominal insert cylinders, not melt/deformation proof','Tab5-to-carrier screws included but female thread geometry/depth unverified','No new protection PCB/cables','No articulated legs or tolerance/strength acceptance']}
...
for row,solid in zip(r['parts'],ss):assert abs(row['volume_mm3']-solid.Volume())<1e-5,row['name']
...
paths={'new_fixed_shell':'validation/tab5_carrier_hardware_v2/fixed_shell.step','new_carrier':'validation/tab5_frame_screws_v2/carrier_installed_approximation.step','new_tray':'validation/lb020_retention_v5/tray.step','new_battery':'validation/lb020_tray_v1/battery.step',**{f'new_strap_{i}':f'validation/lb020_retention_v5/strap_{i}.step' for i in range(2)}}
for sign in [-1,1]:
 for z in [88,112]:
  for kind in ['screw','insert']:paths[f'new_{sign}_{z}_{kind}']=f'validation/tab5_insert_candidate_v1/{sign}_{z}_{kind}.step'
for y in [-60,60]:
 for z in [52,124]:paths[f'new_frame_screw_{y}_{z}']=f'validation/tab5_frame_screws_v2/frame_screw_{y}_{z}.step'
for n,f in paths.items():
 path=ROOT/f;files.append(path);parts[n]=cq.importers.importStep(str(path)).val()
assert len(parts)==115
...
for (n,s),(m,t) in itertools.combinations(parts.items(),2):
 ...
 if v>.01:hits.append({'parts':[n,m],'overlap_mm3':v,'classification':'unresolved_Tab5_thread_envelope' if ('Tab5' in [n,m] and any(x.startswith('new_frame_screw_') for x in [n,m])) else 'unexpected'})
