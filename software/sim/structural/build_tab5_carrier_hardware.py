...
ROOT=Path(__file__).resolve().parents[3];out=ROOT/'validation/tab5_carrier_hardware_v1';out.mkdir(exist_ok=True)
plan={'screw':'NBK SLH-M3-10','nut':'Accu HPN-M3-A2','quantity_each':4,'screw_dimensions_mm':{'length':10,'head_diameter':5.5,'head_height':2,'socket_AF':2},'nut_dimensions_mm':{'AF_max':5.5,'height_max':2.4},'pad_radius_mm':3.5,'pad_abs_y_mm':[61,61.6],'criteria':['Valid single carrier','Pad contacts fixed shell without overlap','Hardware no volume interference with print parts'],'limits':['Nominal envelopes no threads/chamfers','No assembly torque or preload qualified','No screw/head tolerances applied'],'stop':'One nominal stack, report overall width increase explicitly'}
...
for sign in [-1,1]:
 for z in [88,112]:
  ...
  pad=cyl(3.5,.6,61).cut(cyl(1.7,.6,61));pads.append(pad);carrier=carrier.fuse(pad)
