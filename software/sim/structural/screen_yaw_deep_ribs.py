...
source=Path('validation/yaw_shelf_compliance_v1/plan.json');old=json.loads(source.read_text());step=Path('validation/yaw_deep_side_ribs_v1/left_yaw_fixed_support.step')
plan={'question':'Does deepening the side ribs meet the existing 0.2 mm deformation allocation for the archived comparison load?',
 'source_plan':str(source),'source_sha256':hashlib.sha256(source.read_bytes()).hexdigest(),'step_sha256':hashlib.sha256(step.read_bytes()).hexdigest(),
 ...
 'limits':['One archived load only; latest mass and full load envelope absent','Assumed isotropic print material; no certified allowable','Bonded distributed load and ideal rear clamp; no actual joint contact','Not manufacturing release']}
