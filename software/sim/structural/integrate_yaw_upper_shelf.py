...
p=argparse.ArgumentParser(description=__doc__);p.add_argument('--out',type=Path,required=True);p.add_argument('--support-dir',type=Path,default=Path('validation/yaw_upper_shelf_v1'));a=p.parse_args();a.out.mkdir(parents=True,exist_ok=False)
...
for item,prop,solid in zip(inv['parts'],mom['parts'],solids):
 ...
 if name in ['left_yaw_fixed_support','right_yaw_fixed_support']:
  path=a.support_dir/(name+'.step');files.append(path);new=cq.importers.importStep(str(path)).val();assert new.isValid() and len(new.Solids())==1
  changed.append({'name':name,'added_volume_mm3':new.Volume()-solid.Volume(),'actual_mass_delta_g':None})
  solid=new;item.update(volume_mm3=solid.Volume(),source=str(path),source_sha256=hashlib.sha256(path.read_bytes()).hexdigest(),note='Upper reinforcement comparison; unqualified printed material')
