...
def main():
    ...
    package=Path('validation/fast_turn_development_v1/packaging_cad_v5')
    ...
    for sign,index in [(-1,4),(1,5)]:
        old_strip=base.find('geom[@name="col_body_shroud_'+str(index)+'"]')
        base.remove(old_strip)
        for k,(xmin,xmax,ymin,ymax) in enumerate([(-64,6,51,64),(23,51.6,51,64),(6,23,53,64)]):
            ET.SubElement(base,'geom',name=f'col_body_shroud_relief_{index}_{k}',type='box',
                          pos=fmt([(xmin+xmax)/2000,sign*(ymin+ymax)/2000,.0009]),
                          size=fmt([(xmax-xmin)/2000,(ymax-ymin)/2000,.0009]),**{'class':'collision'})
    ...
    for side,sign in [('left',1),('right',-1)]:
        ...
        plate.set('size','.017 .011 .0015');plate.set('pos',fmt(np.array([-.017,sign*.026,.0529])-pivot))
        moving=[]
        ...
        wall=ankle.find('geom[@name="col_'+side+'_boot_shell_2"]')
        wall.set('size','.035 .00075 .001');wall.set('pos',fmt([.002,-sign*.017,-.0148]))
