...
def main():
    ...
    p.add_argument('--residual-scale-rad',type=float,default=.02,
                   help='joint target offset at action=1; motor limits remain unchanged')
    a=p.parse_args()
    ...
    if not np.isfinite(a.residual_scale_rad) or not 0<a.residual_scale_rad<=.08:
        p.error('residual target scale must be in (0,0.08] rad')
    ...
    report={'scope':'Signed reference and unassisted development probe; not maneuver acceptance',
            ...
            'residual_scale_rad':a.residual_scale_rad,
    ...
    config['residual_scale_rad']=a.residual_scale_rad
