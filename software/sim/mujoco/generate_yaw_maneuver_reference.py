...
def main():
    ...
    p.add_argument('--backward-shift-fraction',type=float,help='optional separate COM-transfer fraction for backward steps')
    p.add_argument('--backward-settle-fraction',type=float,default=.1,help='fraction of each backward step reserved after swing touchdown')
    ...
    p.add_argument('--forward-stance-width-mm',type=float,help='place swing feet at this separation during established forward motion; restore initial width otherwise')
    ...
    original_stance_width=float(initial.initial_feet[0,1]-initial.initial_feet[1,1])*1000
    if a.stance_width_mm is not None:
        if not np.isfinite(a.stance_width_mm) or not 58<=a.stance_width_mm<=70:p.error('stance width must be in [58,70] mm')
        center=float(initial.initial_feet[:,1].mean())
    ...
    planner=YawCommandReference(initial,legacy.pose,core.smooth,protocol,kin,a.shift_fraction,a.steady_inset_mm,a.period,a.forward_period,a.swing_profile,a.forward_stance_width_mm,a.backward_settle_fraction,a.backward_shift_fraction)
    ...
    report={'scope':__doc__,'planning_failure':failure,'requested_duration_s':a.duration,'last_reference_time_s':reference[-1]['time_s'] if reference else None,
            ...
            'backward_settle_fraction':a.backward_settle_fraction,
            'backward_shift_fraction':a.shift_fraction if a.backward_shift_fraction is None else a.backward_shift_fraction,
            'backward_swing_fraction':1-a.backward_settle_fraction-(a.shift_fraction if a.backward_shift_fraction is None else a.backward_shift_fraction),
            'forward_stance_width_mm':a.forward_stance_width_mm,
            ...
            'forward_period_s':a.period if a.forward_period is None else a.forward_period,
