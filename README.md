...
**WASDで操作（既定r9）:** `python teleop_yaw.py` — W/Sで前進・後退、A/Dで旋回。同時押しで曲線歩行、Shift＋WASDで向きを保った前後左右・斜めの並進。周期平均速度をフィードバック。キーを離すかSpaceで停止。[速度追従の仕様・評価](docs/VELOCITY_CONTROL_ja.md)。旧r8は `--profile r8`。[操作手順](docs/TELEOP_ja.md)
